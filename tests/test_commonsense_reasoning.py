...
from geniusrise_openai import OpenAICommonsenseReasoningFineTuner
...
def test_load_dataset(bolt, dataset_file):
    ...
    assert dataset[0]["premise"] == "premise_0"
    assert dataset[0]["hypothesis"] == "hypothesis_0"
    assert str(dataset[0]["label"]) == "0"
...
def test_prepare_fine_tuning_data(bolt, dataset_file):
    ...
    assert train_data[0] == '{"prompt":"premise_0\\nhypothesis_0","completion":"0"}'
    assert train_data[1] == '{"prompt":"premise_1\\nhypothesis_1","completion":"1"}'
